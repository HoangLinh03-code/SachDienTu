...
for item in listL:
    sbt_folder = os.path.join(folder_path, item, f"{item}_SBT")
    sgk_folder = os.path.join(folder_path, item, f"{item}_SGK đã fix")
    output_path = os.path.join(folder_path, item, f"{item}_SBT đã fix")
        ...
        for root, dirs, files in os.walk(sbt_folder):
            for f in files:
                if f.lower().endswith(".pdf"):
                    ...
                    sgk_json_path = os.path.join(f"{sgk_folder}\{file_name}\{file_name}", f"{file_name}.json")
